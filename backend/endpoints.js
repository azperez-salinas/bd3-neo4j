...
module.exports = function createEndpoints(runQuery) {
  ...
  router.get(
    ...
    route(async (req, res) => {
      ...
      const to = req.query.to || "Joaquin Phoenix";
      const rows = await runQuery(
        `MATCH (a:Person {name: $from}), (b:Person {name: $to}),
              path = shortestPath((a)-[:COSTARRED_WITH*]-(b))
        RETURN [n IN nodes(path) | n.name] AS actorsPath,
                [r IN relationships(path) | r.movies] AS connectingMovies,
    ...
    route(async (req, res) => {
      const rows = await runQuery(
        ...
        WHERE m.rating >= 7.5 AND m.year >= (maxYear - 10)
         WITH co, collect(DISTINCT m.title) AS successfulMovieTitles
         RETURN co.name AS company,
           size(successfulMovieTitles) AS successfulMovies,
           successfulMovieTitles
  ...
  // Lista de actores que tienen al menos una coincidencia de cast (COSTARRED_WITH),
  // exclusivamente para poblar los <select> de "saltos de separación" en el frontend.
  // No es una de las 15+5 consultas del TP, es soporte de UI.
  router.get(
    "/people/actors",
    route(async (req, res) => {
      const rows = await runQuery(
        `MATCH (p:Person)-[:COSTARRED_WITH]-(:Person)
