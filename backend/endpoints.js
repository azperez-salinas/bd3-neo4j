...
module.exports = function createEndpoints(runQuery) {
  ...
  router.get(
    ...
    route(async (req, res) => {
      const rows = await runQuery(
        `MATCH (a:Person {name: "Jamie Foxx"})-[:PLAYED]->(:Character)-[:APPEARS_IN]->(movie:Movie)
           <-[:APPEARS_IN]-(:Character)<-[:PLAYED]-(b:Person {name: "Leonardo DiCaprio"})
          RETURN [a.name, b.name] AS path,
            1 AS degreesOfSeparation,
            collect(DISTINCT movie.title) AS connectingMovies`,
      );
      res.json(rows[0] || null);
    }),
  );
